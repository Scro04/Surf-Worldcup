...
.controller('compCtrl', function ($scope) {
  ...
  var id = document.getElementById('bewerbe').getAttribute('value');

  console.log("id " + id);

  $scope.header = bewerbeDataSet_[id][1].split("-");

  $scope.descr = bewerbeDataSet_[id][4];

  //------------------------- Rider -----------------------------	
  var tmp_array = [];
  var comp = parseInt(id) + 1;

  console.log(riderDataSet_);
  for (var x = 0; x < riderDataSet_.length; x++) {
    if (riderDataSet_[x][6] == comp) {
